/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ApplicationSupportDetailWhereInput } from "./ApplicationSupportDetailWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class ApplicationSupportDetailListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => ApplicationSupportDetailWhereInput,
  })
  @ValidateNested()
  @Type(() => ApplicationSupportDetailWhereInput)
  @IsOptional()
  @Field(() => ApplicationSupportDetailWhereInput, {
    nullable: true,
  })
  every?: ApplicationSupportDetailWhereInput;

  @ApiProperty({
    required: false,
    type: () => ApplicationSupportDetailWhereInput,
  })
  @ValidateNested()
  @Type(() => ApplicationSupportDetailWhereInput)
  @IsOptional()
  @Field(() => ApplicationSupportDetailWhereInput, {
    nullable: true,
  })
  some?: ApplicationSupportDetailWhereInput;

  @ApiProperty({
    required: false,
    type: () => ApplicationSupportDetailWhereInput,
  })
  @ValidateNested()
  @Type(() => ApplicationSupportDetailWhereInput)
  @IsOptional()
  @Field(() => ApplicationSupportDetailWhereInput, {
    nullable: true,
  })
  none?: ApplicationSupportDetailWhereInput;
}
export { ApplicationSupportDetailListRelationFilter as ApplicationSupportDetailListRelationFilter };
