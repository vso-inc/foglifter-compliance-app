/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AttestationWhereUniqueInput } from "./AttestationWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { AttestationUpdateInput } from "./AttestationUpdateInput";

@ArgsType()
class UpdateAttestationArgs {
  @ApiProperty({
    required: true,
    type: () => AttestationWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => AttestationWhereUniqueInput)
  @Field(() => AttestationWhereUniqueInput, { nullable: false })
  where!: AttestationWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => AttestationUpdateInput,
  })
  @ValidateNested()
  @Type(() => AttestationUpdateInput)
  @Field(() => AttestationUpdateInput, { nullable: false })
  data!: AttestationUpdateInput;
}

export { UpdateAttestationArgs as UpdateAttestationArgs };
